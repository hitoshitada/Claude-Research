...
※ 1記事（12〜18行）の中で最低4種類以上の感情コードを使うこと。

## 原稿の構成
【STEP1: まず採用記事を全件通読し、各記事の要点を把握してから書き始めること】

1. イントロダクション（600〜700字 / 約1〜2分・有料コンテンツへの呼び込み用テザー）
   - F[H]またはF[N]で「{番組タイトル}の時間です」を含む挨拶
   - M[N]またはM[S]で今週のニュース一覧を1件1文でリストアップ（各記事の要点のみ・詳細は本編へ）
   - F[Q]またはM[Q]で「どんな視点で分析するか」の予告（業界動向の切り口・注目ポイント）
   - F[N]またはF[S]で「では詳しく見ていきましょう」と本編へ誘導して締める
   - ※ イントロだけを聴いた人が「本編も聴きたい」と思わせること。詳細・分析・示唆は本編に残す。

2. 本編：各記事の詳細解説（記事1件につき12〜18行 / 700〜900字）
   - F[Q]でスージーが「今週最初のニュースはなんですか？」のように切り出す
   - M[N]/M[S]でトロイがニュースの内容（What）を説明
   - F[Q]でスージーが「業界にとってはどんな意味があるんですか？」と深掘り
   - M[S]でトロイが技術・業界トレンドへのインパクトを解説
   - F[Q]またはF[S]で「企業の動きや投資への示唆は？」と展開
   - M[N]またはM[S]でトロイが企業動向・今後の見通しを解説
   - F[N]/F[S]でスージーが受けて次の記事へ橋渡し

3. クロージング（400〜500字）
   - M[S]またはM[Q]で今週全体のトレンドを1〜2文でまとめる
   - F[S]またはF[H]で締めの言葉＋「{番組タイトル}の時間でした」を含めること

## 分量
- イントロダクション: 600〜700字（全体の約14%）
- 本編・記事1件あたり: 700〜900字（12〜18行）
- クロージング: 400〜500字
- 全体目標: 約4,800字（15分 × 約320字/分）
- 記事が3件以下の場合は各記事を900〜1,000字に増やして調整すること
- 記事が7件以上の場合は各記事を600〜700字に抑えてイントロのリストアップも1文ずつに収めること
...
def load_podcast_prompt(topic_name: str) -> str:
    ...
    # ── 番組タイトルフレーズ＋構成・分量を先頭に注入（最重要ルールとして最初に提示）──
    # トピック固有ファイルの古い構成・分量指示より優先させるため先頭に置く
    ...
    title_rule = (
        ...
        f"\n"
        f"## 【最重要】原稿の構成と分量（以下の指示がファイル内の他の構成指示より優先されます）\n"
        f"【STEP1: まず採用記事を全件通読し、各記事の要点を把握してから書き始めること】\n"
        f"\n"
        f"### 1. イントロダクション（600〜700字 / 約1〜2分・有料コンテンツへの呼び込み用テザー）\n"
        f"- F[H]またはF[N]で「{program_title}の時間です」を含む挨拶\n"
        f"- M[N]またはM[S]で今週のニュース一覧を1件1文でリストアップ（要点のみ・詳細は本編へ）\n"
        f"- F[Q]またはM[Q]で「どんな視点で分析するか」の予告（業界動向の切り口・注目ポイント）\n"
        f"- F[N]またはF[S]で「では詳しく見ていきましょう」と本編へ誘導して締める\n"
        f"- ※ イントロだけを聴いた人が「本編も聴きたい」と思わせること。詳細・分析・示唆は本編に残す。\n"
        f"\n"
        f"### 2. 本編：各記事の詳細解説（記事1件につき12〜18行 / 700〜900字）\n"
        f"- F[Q]でスージーが切り出し → M[N]/M[S]でトロイがニュース内容（What）を説明\n"
        f"- F[Q]でスージーが「業界にとってどんな意味がありますか？」と深掘り\n"
        f"- M[S]でトロイが技術・業界トレンドへのインパクトを解説\n"
        f"- F[Q]/F[S]で「企業の動きや今後の見通しは？」と展開\n"
        f"- M[N]/M[S]でトロイが企業動向・今後の見通しを解説\n"
        f"- F[N]/F[S]でスージーが受けて次の記事へ橋渡し\n"
        f"\n"
        f"### 3. クロージング（400〜500字）\n"
        f"- M[S]またはM[Q]で今週全体のトレンドを1〜2文でまとめる\n"
        f"- F[S]またはF[H]で締めの言葉＋「{program_title}の時間でした」を含めること\n"
        f"\n"
        f"### 分量目標\n"
        f"全体: 約4,800字（15分 × 約320字/分）\n"
        f"記事3件以下: 各記事900〜1,000字に増量　記事7件以上: 各記事600〜700字に抑制\n"
    )
